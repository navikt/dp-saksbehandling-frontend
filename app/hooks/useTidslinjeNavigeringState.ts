...
export function useTidslinjeNavigeringState(
  ...
  const [antallUkerITidslinje, setAntallUkerITidslinje] = useState<
    AntallUkerITidslinje
  >("2");
  const [tidslinjeStartSlutt, setTidslinjeStartSlutt] = useState<
    TidslinjeStartSlutt
  >({
    start: førsteFraOgMedDato
      ? sub(new Date(førsteFraOgMedDato), { days: 1 })
      : new Date(),
    end: add(new Date(førsteFraOgMedDato ?? new Date()), {
