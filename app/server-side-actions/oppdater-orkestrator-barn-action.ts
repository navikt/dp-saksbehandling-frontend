...
export async function oppdaterOrkestratorBarnAction(
  request: Request,
  formData: FormData,
) {
  ...
  const kvalifisererTilBarnetillegg = formData.get(
  ...
  const oppdatertBarn: components["schemas"]["OppdatertBarnRequest"] = {
    ...
    oppdatertBarn: {
      ...
      etternavn,
      fodselsdato: format(
        parse(fodselsdato, "dd.MM.yyyy", new Date()),
        "yyyy-MM-dd",
      ),
      oppholdssted,
      forsorgerBarnet: forsorgerBarnet === "true",
      kvalifisererTilBarnetillegg: kvalifisererTilBarnetillegg === "true",
      barnetilleggFom: format(
        parse(barnetilleggFom, "dd.MM.yyyy", new Date()),
        "yyyy-MM-dd",
      ),
      barnetilleggTom: format(
        parse(barnetilleggTom, "dd.MM.yyyy", new Date()),
        "yyyy-MM-dd",
      ),
      begrunnelse,
    },
  ...
  const { error } = await oppdaterOrkestratorBarn(
    request,
    soknadId,
    oppdatertBarn,
  );
